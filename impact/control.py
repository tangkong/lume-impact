...
class ControlGroup:
    ...
    def __init__(
        ...
        assert (
            len(self.attributes) == n_ele
        ), "attributes should be a list with the same length as ele_names"
        ...
        assert (
            len(self.factors) == n_ele
        ), "factors should be a list with the same length as ele_names"
    ...
    def ele_values(self):
        ...
        return [
            self.ele_dict[ele_name][attrib]
            for ele_name, attrib in zip(self.ele_names, self.attributes)
        ]
    ...
    def set_delta(self, key, item):
        ...
        self.value = item
        for name, attrib, f, ref in zip(
            self.ele_names, self.attributes, self.factors, self.reference_values
        ):
